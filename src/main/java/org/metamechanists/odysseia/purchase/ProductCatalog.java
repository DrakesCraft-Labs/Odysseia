...
public final class ProductCatalog {
    ...
    public ProductDefinition get(String id) {
        ...
        ProductDefinition direct = products.get(id);
        if (direct != null) return direct;

        String lower = id.toLowerCase(Locale.ROOT).trim();
        for (Map.Entry<String, ProductDefinition> entry : products.entrySet()) {
            if (entry.getKey().equalsIgnoreCase(lower)) return entry.getValue();
        }

        String prefixed = lower.startsWith("vip_") ? lower : "vip_" + lower;
        String unprefixed = lower.startsWith("vip_") ? lower.substring(4) : lower;
        for (Map.Entry<String, ProductDefinition> entry : products.entrySet()) {
            String key = entry.getKey().toLowerCase(Locale.ROOT);
            if (key.equals(prefixed) || key.equals(unprefixed)) return entry.getValue();
        }

        try {
            int packageId = Integer.parseInt(lower);
            for (ProductDefinition product : products.values()) {
                if (product.tebexPackageId() == packageId) return product;
            }
